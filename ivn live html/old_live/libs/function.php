...
define("THANKYOU_SIGNUP",'Tak for din tilmelding.');
...
function start_forget_password($email){
    ...
    $to  = $email;
    $subject = 'IVN | Opret nyt password';
    $from = 'no-reply@ivn.dk';
	generic_email_sender($to , $subject, $message,$from,CHECK_EMAIL_TO_RESET_PASS,EMAIL_ERROR);
}
...
function generate_verification_email($user_id){
	...
	//Generating Message
	$message= '<h3>Velkommen til IVN.dk</h3>';
	$message.='<br><p>Du har for nyligt oprettet en profil på siden, og vi glæder os til at vise dig, hvad vi kan tilbyde.</p>';
    ...
    $to  = $email;
    $subject = 'Velkommen til IVN.dk';
    $from = 'no-reply@ivn.dk';
	generic_email_sender($to , $subject, $message,$from,CHECK_EMAIL_TO_VERIFY,EMAIL_ERROR);
}
function generic_email_sender($to,$subject,$message,$from,$success_message,$fail){
...
$mail = $smtp->send($email, $headers, utf8_decode($body));
