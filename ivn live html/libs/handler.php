	...
	$action		= mysql_real_escape_string($_POST['action']);
		switch($action){
			...
			case 'SIGNIN':
					...
					$row      = get_row('users',"where email='$login_email'");
					...
					//Check If Fields are empty
						if(empty($login_email) || empty($login_password)){
							...
							//Login Authenticate
							if(login_authenticate($login_email)){
									...
									//Check if user is verified
									if(!$result['verified']){
										die(USER_IS_NOT_VERIFIED);
									}
									if(validate_password($salt,$login_password,$password)):
												session_start();
												//generating user token
												$new_salt  = generateSalt();
												$user_token = generateHashWithSalt($email,$new_salt);
												$new_token = array('user_token' => $user_token);
												//Storing Verification Code
												dbRowUpdate('users',$new_token, "where id = $id");
			...
			case 'PROFILE_UPDATE':
					...
					$primary_email 		= mysql_real_escape_string($_POST['p_email']);
					// $p_email		= mysql_real_escape_string ($_POST['u_email']);
					// if (!filter_var($primary_email, FILTER_VALIDATE_EMAIL) && !empty($primary_email)) {
					// 	die(NOT_VALID_EMAIL);
					// }
					// if(!empty($primary_email)){
					// 		if(Dubplicate('users','email',$primary_email)){

					// 				if(!primary_check('users','email',$primary_email,$p_email)){
					// 					die('Primary Email Already Taken');
					// 				}
					// 		}
					// }
					//Passwords Block
			...
			case 'CONTACT-US':
					...
					$to = 'info@ivn.dk';
					$subject =  'IVN | Besked';
					$message ="<p>Navn: $contact_name</p>";
					$message.="<br><p>Email: $contact_email</p>";
					$message.="<br><p>Besked: $contact_msg</p>";
					$from = 'no-reply@ivn.dk';
					generic_email_sender($to , $subject, $message,$from,CONTACT_YOU_SHORTLY,EMAIL_ERROR);
			...
			case 'SUBSCRIBE':
					...
					$to = 'jh@salgschef.dk';
					$subject =  'Tilmelding til Virtuel Salgschef';
					$message = '<p>Navn:'.$sub_name.'</p><br><p>E-mail: '.$sub_email.'</p><br><p>Virksomhed: '.$sub_company.
					'</p>';
					$from = 'no-reply@ivn.dk';
					generic_email_sender($to , $subject, $message,$from,THANKYOU_SIGNUP,EMAIL_ERROR);

					
				break;
			case 'website-eller-webshop':
					//geting variables
					...
					$to = 'salg@thagaard.org ';
					$subject =  'website-eller-webshop';
					$message = '<p>Navn:'.$name.'</p><br><p>E-mail: '.$email.'</p><br><p>Type: '.$type.'</p><br><p>Cvr: '.$cvr.'</p><br><p>Comment: '.$comment.
					'</p>';
					$from = 'no-reply@ivn.dk';
					generic_email_sender($to , $subject, $message,$from,THANKYOU_SIGNUP,EMAIL_ERROR);
				...
				case 'Second_Subscribe':
					//geting variables

					$sub_fulde 	 = mysql_real_escape_string($_POST['sub_fulde']);
					$sub_email   = mysql_real_escape_string($_POST['sub_email']);
					$sub_company = mysql_real_escape_string($_POST['sub_company']);
					$sub_cvr 	 = mysql_real_escape_string($_POST['sub_cvr']);
					$sub_hje     = mysql_real_escape_string($_POST['sub_hje']);
					$sub_tel     = mysql_real_escape_string($_POST['sub_tel']);
				
					if((empty($sub_company)) || (empty($sub_fulde)) || (empty($sub_cvr)) || 
						(empty($sub_hje)) || (empty($sub_tel))){
						die(ALL_FIELDS_REQUIRED);
						
					}if (!filter_var($sub_email, FILTER_VALIDATE_EMAIL)) {
							die(NOT_VALID_EMAIL);
					}
					$to = 'support@pensopay.com';
					$subject =  'Tilmelding til Pensopay';
					
					$message = '<p>Virksomhed: '.$sub_company.'</p>';
					$message.='<br><p>Navn: '.$sub_fulde.'</p>';
					$message.='<br><p>E-mail: '.$sub_email.'</p>';
					$message.='<br><p>CVR: '.$sub_cvr.'</p>';
					$message.='<br><p>Website: '.$sub_hje.'</p>';
					$message.='<br><p>Telefon: '.$sub_tel.'</p>';
				...
				case 'JURA_SUBSCRIBE':
				//geting variables
					$name    = mysql_real_escape_string($_POST['sub_name']);
					$email   = mysql_real_escape_string($_POST['sub_email']);
					$company = mysql_real_escape_string($_POST['sub_company']);
					...
					$to = 'kontakt@kvikadvokat.dk';
					$subject =  'Tilmelding til Advokatrådgivning';

					$message = '<p>Navn:'.$name.'<br>E-mail: '.$email.'<br>Virksomhed: '.$company.'</p>';
					$from = 'no-reply@ivn.dk';
					generic_email_sender($to , $subject, $message,$from,THANKYOU_SIGNUP,EMAIL_ERROR);
					
					//generat_jura_email($sub_name,$sub_email,$sub_company);

				case 'SUBSCRIBE_ENT':
					$name 		= mysql_real_escape_string($_POST['sub_name']);
					$phone 		= mysql_real_escape_string($_POST['sub_phone']);
					$email 		= mysql_real_escape_string($_POST['sub_email']);
					$company 	= mysql_real_escape_string($_POST['sub_company']);
					$more_about = mysql_real_escape_string($_POST['more_about']);
					...
					$to = 'salg@lendino.dk';
					$subject = 'Lendino forespørgsel';
					$message = '<p>Navn: '.$name.'</p>';
					$message.='<br /><p>Telefon:'.$phone.'</p>';
					$message.='<br /><p>E-mail:'.$email.'</p>';
					$message.='<br><p>Virksomhed: '.$company.'</p>';
					$message.='<br><p>Jeg vil gerne høre mere om:'.$more_about.'</p>';
					$from = 'no-reply@ivn.dk';
					generic_email_sender($to , $subject, $message,$from,'Tak for din henvendelse.',EMAIL_ERROR);
				...
				case 'VERIFY_IVN':
					...
					$login_email 	= mysql_real_escape_string($_POST['login_email']);
					$login_password = mysql_real_escape_string($_POST['login_password']);
					$fb_email		= mysql_real_escape_string($_POST['fb_email']);
						...
						if(empty($login_email) || empty($login_password)){
							...
							if(login_authenticate($login_email)){
									...
									if(validate_password($salt,$login_password,$password)):
										...
										if(dbRowUpdate('users',$form_data," email = '$email'")){
											echo FACEBOOK_MERGE.'</br>';
										}else{
											die(MERGE_ERROR);
										}
												session_start();
			...
			break;
			case 'NOT_MERGE':
				$first_name 		= mysql_real_escape_string($_POST['fb_first_name']);
				$last_name 			= mysql_real_escape_string($_POST['fb_last_name']);
				$email				= mysql_real_escape_string($_POST['fb_email']);
				if(empty($first_name) || empty($last_name) || empty($email)){
								die(ALL_FIELDS_REQUIRED);
				}
				if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
